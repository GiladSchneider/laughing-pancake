from astar_algo_badTieBreak import loop1
...
def r_astar(board, xstart, ystart, xtar, ytar, n, num):
    ...
    #Start the game loop
    path = loop1(copy.deepcopy(game_board), state_x, state_y, goal_x, goal_y, n, num)
    ...
    while not found:
        ...
        for s in reversed(list(range(len(path)))):
            ...
            if board[to_x][to_y] == 'B':
                #print('ERR 2')
                path = loop1(copy.deepcopy(game_board), state_x, state_y, xtar, ytar, n, num)
                break
...
for i in range(50):
        ...
        calcOps(1) #data1 corresponds to the operations from repeated a*
t2=time.perf_counter()
print("bad tiebreaking repeated A* took "+ str(t2-t1)+ " seconds")
