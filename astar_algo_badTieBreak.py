...
def siftup(tree,i,dict,g):
    while i>0:
        if dict[tree[i]]<dict[tree[i//2]] or (dict[tree[i]]==dict[tree[i//2]] and g[tree[i//2]]>g[tree[i]]):#look at g tree{i} instead #look at lines 10 and 25, this is tie breaking so lower g value gets priority
            temp = tree[i//2]
            tree[i//2] = tree[i]
            tree[i] = temp
        i=i//2
